 ...
 *
 * Proves 35 causal negatives (5 tests × 7 publication routes).
 * Routes under test:
 ...
 *   POST /api/v1/facebook/generate_and_publish      (facebook.ts)
 *   POST /api/render-jobs/:jobId/dead-letter/retry  (renderJobs.ts, admin-gated)
 *
 * N4 tests prove GENUINE non-circular binding: source_asset_id is supplied
 * as an independent field in the request body. When it differs from
 * rights_record.source_asset_id ("asset-test-123"), the server returns
 * 403 JOB_ASSET_ID_MISMATCH — proving verifyJobRightsBinding compares
 * independently-supplied identity against the rights record.
 *
...
vi.mock("../../server/services/facebookService", () => ({
  validateFacebookCredentials: async () => ({ valid: true }),
  publishToFacebook: async () => ({ postId: "fb-post-id" }),
}));
...
const MISMATCH_ASSET_ID = "DIFFERENT-ASSET-ID-THAT-DOES-NOT-MATCH";
...
describe("POST /api/batch-export — RightsAndConsentV2 enforcement", () => {
  ...
  it("POSITIVE: matching source_asset_id + eligible rights_record → 2xx", async () => {
  ...
  it("N4: source_asset_id in body differs from rights_record → 403 JOB_ASSET_ID_MISMATCH", async () => {
    const res = await request(app)
      .post("/api/batch-export")
      .send({ ...baseBody, source_asset_id: MISMATCH_ASSET_ID, rights_record: eligibleRights() });
    expect(res.status).toBe(403);
    expect(res.body.binding_errors).toContain("JOB_ASSET_ID_MISMATCH");
  });
});
...
// ═══════════════════════════════════════════════════════════════════════════════
// POST /api/v1/facebook/generate_and_publish
// ═══════════════════════════════════════════════════════════════════════════════

describe("POST /api/v1/facebook/generate_and_publish — RightsAndConsentV2 enforcement", () => {
  let app: Express;

  beforeEach(async () => {
    const facebookRouter = (await import("../../server/routes/facebook")).default;
    app = express();
    app.use(express.json());
  ...
  const baseBody = {
    prompt: "A test Facebook video prompt",
    page_id: "fb-page-123",
    source_asset_id: "asset-test-123",
    source_checksum: CHECKSUM,
  };
  ...
  it("N1: missing rights_record → 400", async () => {
    const res = await request(app).post("/api/v1/facebook/generate_and_publish").send(baseBody);
    expect(res.status).toBe(400);
  });
...
// Does NOT use independent source_asset_id — emptyBodySchema applies.
// ═══════════════════════════════════════════════════════════════════════════════
